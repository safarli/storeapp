  ...
  <div>
    ...
    <form>
      ...
      <input
        ...
        type="text"
        id="productid"
        placeholder="ex: 100334"
        required
      />
      <button type="sumbit" @click="findRecord($event, 'salam')">Find</button>
    </form>
    <form>
      <div>
        <label for="productId">ID:</label>
        <input v-model="foundId" id="productId" type="text" required />
      </div>
      <div>
        <label for="productName">Name:</label>
        <input v-model="foundName" id="productName" type="text" required />
      </div>
      <div>
        <label for="productStock">Stock:</label>
        <input v-model="foundStock" id="productStock" type="text" required />
      </div>
...
export default {
  ...
  methods: {
    findRecord: function(evn, str) {
      evn.preventDefault();
      console.log(str);
      axios
        .get(`http://localhost:3000/searchbyid/${this.productId}`)
        .then(result => {
          console.log(result);
          (this.foundId = result.data[0].id),
            (this.foundName = result.data[0].productname),
            (this.foundStock = result.data[0].stock);
        })
